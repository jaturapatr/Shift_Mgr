...
from shift_manager.models import Employee, Team, DailyRequirement, ShiftBlock, MachineConstraint

load_dotenv()

def run_end_to_end_stress_test():
    print("🚀 STARTING END-TO-END STRESS TEST")
    print("=" * 50)
    ...
    # 2. DEFINE DIVERSE NL INSTRUCTIONS
    instructions = [
    ...
    print("\n[PHASE 1] Translating Human Language to Machine Logic (BATCH MODE)...")
    all_translated_constraints = translator.translate_batch(instructions, employees, teams, company_id)
    
    if not all_translated_constraints:
        print("    ⚠️ WARNING: No constraints generated!")

    print("\n[SUMMARY] Total Constraints Generated:", len(all_translated_constraints))
    ...
    for i in range(7):
        ...
        blocks = [ShiftBlock(start_hour=h*4, team_requirements={}) for h in range(6)]
        requirements.append(DailyRequirement(date=curr_date, blocks=blocks))

    print("\n[PHASE 2] Running OR-Tools Solver with Translated Logic...")
    ...
    if score is not None:
        ...
        print(f" - Bob's Work Days: {bob_days} {'✅ (Limit Respected)' if bob_days <= 3 else '❌ (FAILED)'}")
